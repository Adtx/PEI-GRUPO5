...
public class ListPEIMVPData {
	...
	public void listTestData() throws PersistentException {
		...
		System.out.println("Listing Context...");
		Context[] contexts = ContextDAO.listContextByQuery(null, null);
		length = Math.min(contexts.length, ROW_COUNT);
		for (int i = 0; i < length; i++) {
	...
	public void listByCriteria() throws PersistentException {
		...
		System.out.println("Listing Context by Criteria...");
		ContextCriteria contextCriteria = new ContextCriteria();
		// Please uncomment the follow line and fill in parameter(s) 
		//contextCriteria.ID.eq();
		contextCriteria.setMaxResults(ROW_COUNT);
		Context[] contexts = contextCriteria.listContext();
		length =contexts== null ? 0 : Math.min(contexts.length, ROW_COUNT); 
		for (int i = 0; i < length; i++) {
			 System.out.println(contexts[i]);
		}
		System.out.println(length + " Context record(s) retrieved."); 
